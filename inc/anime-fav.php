...
try {
    ...
    $stmt = $pdo->prepare('SELECT id FROM animelike WHERE username = ? ');
...
if (isset($total)) {
    ...
    foreach ($idArrayA as $idAnime) {
        $responseA = file_get_contents('https://kitsu.io/api/edge/anime?filter[id]=' . $idAnime);
            ...
            if (empty($mainTargetA->posterImage->$size)) {
                if (empty($mainTargetA->coverImage->$size)) {
                    ...
                    break;
                        ...
                        $imgA[] = 'http://atlas-content-cdn.pixelsquid.com/stock-images/broken0-heart-z0VDl3D-600.jpg';
                        break;
                } else {
...
if (!isset($total)) {
    ...
    <div class='container max'>    
        <h2> There is so much you can like in the Anime section the more you like the more will show. </h2>
...
?>
    <div class="container">
    <h2 class="header">Liked Anime</h2>
        ...
        ?>
            <div class="card" >  <!--card 1--> 
	            <div class="img-section">
		            <img class="card-img-top" src="<?php  echo $imgA[$i]; ?>" alt="dummy">
	            </div>
	            <div class="card-body">
		            <h5 class="card-title"><?php echo $titleA[$i]; ?></h5>
		            <p class="card-text"><?php  echo substr($synopsisA[$i], 0, 200)."..."; ?></p>
				    <a href="<?php echo "https://kitsu.io/anime/" . $idA[$i]; ?>" class="btn btn-primary btn-card" target="_blank">View on Kitsu</a>
					<div class="anime">
						<div class="animeStar">
							<span data-id="<?php echo substr($linkTargetA[$i]->self, 32 ); ?>" class="fa fa-star fa-2x active_star"></span>
						</div>
					</div>
                </div>
           </div>
            ...
            <?php
                    ...
                    // Do we have any results?
                        ...
                        // Display the results
                        for ($i = 1; $i <= $pages; $i++) {
                            echo "<a href='anime-fav.php?page=$i'> $i </p> </span>";
                        }
            ?>
